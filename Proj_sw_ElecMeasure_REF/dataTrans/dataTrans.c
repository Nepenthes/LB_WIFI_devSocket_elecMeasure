	...
	switch(repeatTX_Len){
		...
		case dataTransLength_objSERVER:{
...
#else
			memcpy(&dats_Tx[7],  &MAC_ID[0], 6);	/*老版APP全MAC填充*///远端MACID填充
#endif
			...
			if(!ifSpecial_CMD)dats_Tx[10 + 12] = SWITCH_TYPE_FP;	//开关类型填充
				...
				dats_Tx[4 + 12] = frame_Check(&dats_Tx[5 + 12], 28);	//校验码		
#else		
				memcpy(&dats_Tx[4 + 12], &MAC_ID[0], 6);	//MAC填充-老版APP MAC地址为6位，从D16开始
#endif
		...
		case dataTransLength_objMOBILE:{
			...
			if(!ifSpecial_CMD)dats_Tx[10] = SWITCH_TYPE_FP;	//开关类型填充
				...
				dats_Tx[4] 	= frame_Check(&dats_Tx[5], 28); //校验码		
#else
				memcpy(&dats_Tx[4], &MAC_ID[0], 6);	//MAC填充-老版APP MAC地址为6位，从D4开始
#endif
...
#endif		
			){ /*服务器回的还是14字节*///但是帧长为20，我发的20字节，服务器依然回14字节
	...
	if(datsQualified_FLG){
		...
		switch(frameLength){
			...
			case dataTransLength_objMOBILE:{
...
#if(SMARTSWITCH_IF == 1)
				if(!memcmp(&rxBuff_WIFI_temp[4], &MAC_ID[0], 6))frameMacCheck_PASS  = 1; //mac地址检查	
#else
				...
				if((rxBuff_WIFI_temp[3] == FRAME_MtoSCMD_cmdConfigSearch) || (rxBuff_WIFI_temp[3] == FRAME_MtoSCMD_cmdQuery)){ //不用进行mac地址检查的指令
				
					frameMacCheck_PASS = 1;	
				}			
#if(SMARTSWITCH_IF == 1)
				frameCodeCheck_PASS = 1; //老版APP不要校验
#endif			
					...
					switch(cmdParing_Temp){		//命令解析及识别
						...
						case FRAME_MtoSCMD_cmdConfigSearch:{	
							...
							if(!deviceLock_IF){		
								...
								memcpy(&repeatTX_buff[14], serverIP_temp, 6); //ip信息
#endif
								//时间信息同步
						...
						case FRAME_MtoSCMD_cmdRemoteServerChg:{
								...
								//服务器地址同步
								WIFI_serverUDP_CHG(&serverIP_temp[2], &serverIP_temp[0]); //下标12、13端口，下标14开始为IP
								respond_FLAG = 0;
								...
								case cmdConfigTim_normalSwConfig:{	/*普通定时*/
									...
									for(loop = 0; loop < 4; loop ++){
										...
										if(rxBuff_WIFI_temp[14 + loop * 3] == 0x80){	/*一次性定时判断*///周占位为空，而定时器被打开，说明是一次性
											...
											rxBuff_WIFI_temp[14 + loop * 3] |= (1 << (rxBuff_WIFI_temp[31] - 1)); //强行进行当前周占位，当次执行完毕后清除 <rxBuff_WIFI_temp[31]为当前授时 周>
										}
									}
...
//			}
			...
			heartBeat_Pack[11]	= (u8)((integer_prt & 0x00FF) >> 0);	//整数部分填装
			heartBeat_Pack[12]	= (u8)((decimal_prt & 0xFF00) >> 8);	//小数部分填装
			heartBeat_Pack[13]	= (u8)((decimal_prt & 0x00FF) >> 0);	//小数部分填装
			}
			...
			heartBeat_Pack[14]	= (u8)((integer_prt & 0xFF00) >> 8);	//整数部分填装
			heartBeat_Pack[15]	= (u8)((integer_prt & 0x00FF) >> 0);	//整数部分填装
			heartBeat_Pack[16]	= (u8)((decimal_prt & 0xFF00) >> 8);	//小数部分填装
			heartBeat_Pack[17]	= (u8)((decimal_prt & 0x00FF) >> 0);	//小数部分填装
			}
			
			heartBeat_Pack[18]	= elec_Consum_moment;	//对应时段标志填装
			...
			heartBeat_Pack[12]	= (ifDelay_sw_running_FLAG & 0x01) >> 0; //绿色模式
		}
		
		heartBeat_Pack[19]	= SWITCH_TYPE_FP;
		heartBeat_Pack[20]	= FRAME_TAIL_HEARTBEAT;
		
#if(SMARTSWITCH_IF != 1) //是否为老版APP
	...
	if(swDataBroadcast_IF){
		...
		dtasTX_loadBasic_CUST( DATATRANS_objFLAG_MOBILE,
							   repeatTX_buff,	/*发送前最后装载*///发送包基本信息填充
							   FRAME_TYPE_StoM_RCVsuccess,
							   FRAME_MtoSCMD_cmdControl,
							   0
							 );
		uartObjWIFI_Send_String(repeatTX_buff, dataTransLength_objMOBILE);//数据接收回复响应	
	}
#endif
}
